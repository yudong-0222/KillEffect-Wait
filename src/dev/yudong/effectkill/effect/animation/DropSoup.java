...
public class DropSoup extends MainEffectKill{
	...
	public DropSoup() {
		super("dropsoup", YAMLUtils.get("messages").getFile().exists()?((String) Utils.gfc("messages", "effectKill.dropsoup.name")):("§a湯品開灑"), new ArrayList<>(Arrays.asList("&a和尚端湯上塔 塔滑湯灑湯燙塔 和尚端塔上湯 湯滑塔灑塔燙湯.",  "&8左鍵點擊來套用特效")), Heads.SOUP.getTexture());
	}
